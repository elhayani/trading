...
OIL_CORRIDORS = {
    # === SESSION PÉTROLE (Règles strictes V5.1) ===
    # Volume Filter augmenté de 1.5x à 2.0x suite au backtest 2025
    'NYMEX_OPEN': {
        'start': (14, 0), 'end': (15, 30),
        'name': '🛢️ NYMEX Pre-Open',
        'regime': MarketRegime.PULLBACK_SNIPER,
        'description': 'Avant ouverture. SEULEMENT si volume extrême.',
        'params': {
            'tp_multiplier': 0.4,
            'sl_multiplier': 0.5,
            'risk_multiplier': 0.8, # Risque réduit
            'rsi_threshold': 35,    # Plus sélectif
            'min_volume_ratio': 1.8, # Volume durci
            'max_trades': 1,
            'scalping_mode': False,
        }
    },
    'NYMEX_CORE': {
        ...
        'name': '🔥 Petrol Core',
        'regime': MarketRegime.AGGRESSIVE_BREAKOUT,
        'description': 'Session active. Breakout VALIDÉ par volume x2.0.',
        'params': {
            'tp_multiplier': 0.6,
            'sl_multiplier': 0.5,
            'risk_multiplier': 1.2,
            'rsi_threshold': 40,
            'min_volume_ratio': 2.0, # 🛑 VETO STRICT 2.0x
            'max_trades': 2,
            'scalping_mode': True,
        }
    },
    'NYMEX_CLOSE': {
        ...
        'name': '🛑 NYMEX Close',
        'regime': MarketRegime.CAUTIOUS_REVERSAL,
        'description': 'Fermeture. Risque minimal.',
        'params': {
            'tp_multiplier': 0.3, # TP très court
            'sl_multiplier': 0.4,
            'risk_multiplier': 0.5, # Demi-risque
            'rsi_threshold': 30,
            'min_volume_ratio': 1.5,
            'max_trades': 1,
            'scalping_mode': False,
...
def get_paris_time(simulated_time=None) -> datetime.datetime:
    """
...
def get_current_corridor(symbol: str, simulated_time=None) -> Optional[Dict]:
    """
...
def get_corridor_params(symbol: str, asset_class: str = None, simulated_time=None) -> Dict:
    """
    🎯 FONCTION PRINCIPALE
    Retourne les paramètres de trading adaptés au corridor actuel.
    """
    corridor = get_current_corridor(symbol, simulated_time)
...
def get_current_regime(symbol: str, simulated_time=None) -> str:
    """
...
def calculate_adaptive_tp_sl(symbol: str, base_tp: float, base_sl: float, simulated_time=None) -> Tuple[float, float]:
    """
    Calcule le TP et SL adaptatifs basés sur le corridor actuel.
    """
    corridor = get_corridor_params(symbol, simulated_time=simulated_time)
...
def should_increase_position_size(symbol: str, simulated_time=None) -> Tuple[bool, float]:
    """
...
def is_scalping_enabled(symbol: str, simulated_time=None) -> bool:
...
def get_max_trades_in_corridor(symbol: str, simulated_time=None) -> int:
...
def check_volume_veto(symbol: str, current_volume: float, avg_volume: float, simulated_time=None) -> dict:
    """
    🛑 VETO DE VOLUME - Point 3 de la Check-list Empire V5.1
    """
    ...
    if avg_volume <= 0:
        return {
            ...
            'volume_ratio': 0.0,
            'min_required': 0.0,
            'reason': 'Volume moyen non disponible - Veto désactivé'
        }
    
    # Calculer le ratio
    volume_ratio = current_volume / avg_volume
    
    # Récupérer le seuil minimum du corridor actuel
    corridor = get_corridor_params(symbol, simulated_time=simulated_time)
    params = corridor.get('params', {})
    min_volume_ratio = params.get('min_volume_ratio', 1.0)
    corridor_name = corridor.get('name', 'Default')
    regime = corridor.get('regime', 'STANDARD')
    
    # Veto si volume insuffisant
    if volume_ratio < min_volume_ratio:
        return {
            'veto': True,
            'volume_ratio': round(volume_ratio, 2),
            'min_required': min_volume_ratio,
            'reason': f'🛑 VETO VOLUME: {volume_ratio:.2f}x < {min_volume_ratio}x requis en {corridor_name}'
        }
    ...
    return {
        ...
        'volume_ratio': round(volume_ratio, 2),
        'min_required': min_volume_ratio,
        'reason': f'✅ Volume OK: {volume_ratio:.2f}x >= {min_volume_ratio}x requis'
    }


def get_adaptive_params(symbol: str, base_params: dict = None, simulated_time=None) -> dict:
    """
    🎯 FONCTION CENTRALE V5.1 - Paramètres Adaptatifs Unifiés
    
    Cette fonction combine tous les ajustements du corridor pour retourner
    les paramètres de trading finaux.
    """
    corridor = get_corridor_params(symbol, simulated_time=simulated_time)
    params = corridor.get('params', {})
    regime = corridor.get('regime', MarketRegime.TREND_FOLLOWING)
    
    # Déterminer le niveau d'agressivité basé sur le régime
    if regime in [MarketRegime.AGGRESSIVE_BREAKOUT, MarketRegime.PULLBACK_SNIPER]:
        aggressiveness = 'HIGH'
    elif regime in [MarketRegime.TREND_FOLLOWING, MarketRegime.SCALPING]:
        aggressiveness = 'MEDIUM'
    else:  # CAUTIOUS_REVERSAL, LOW_LIQUIDITY, CLOSED
        aggressiveness = 'LOW'
    
    adaptive_params = {
        # Multiplicateurs
        'tp_multiplier': params.get('tp_multiplier', 1.0),
        ...
        # Modes
        'scalping_mode': params.get('scalping_mode', False),
        'regime': regime,
        'corridor_name': corridor.get('name', 'Default'),
        'corridor_id': corridor.get('corridor_id', 'DEFAULT'),
        
        # Agressivité (pour le Position Sizing)
        'aggressiveness': aggressiveness,
    }
    
    # Merge avec les paramètres de base si fournis
    if base_params:
        for key, value in base_params.items():
            if key not in adaptive_params:
                adaptive_params[key] = value
