...
def run_backtest():
    print("🚀 Backtest Commodities 2026 (YTD) Started...")
    ...
    for pair, config in CONFIGURATION.items():
        ...
        # 1. Download Data (2026)
        try:
            df = yf.download(pair, start="2026-01-01", end=datetime.now().strftime("%Y-%m-%d"), interval="1h", progress=False)
        ...
        for i in range(1, len(rows)):
            ...
            if open_trade:
                ...
                if sl_hit or tp_hit:
                    pnl_raw = (open_trade['sl'] - open_trade['entry']) if sl_hit else (open_trade['tp'] - open_trade['entry'])
                    if open_trade['type'] == 'SHORT':
                         pnl_raw = (open_trade['entry'] - open_trade['sl']) if sl_hit else (open_trade['entry'] - open_trade['tp'])
                         
                    print(f"      Trade Closed: {open_trade['type']} @ {open_trade['entry']:.2f} -> {open_trade['sl' if sl_hit else 'tp']:.2f} | PnL: {pnl_raw:.2f}")
