...
class ForexStrategies:
    ...
    def check_signal(pair, df, config):
        ...
        # Check special flags
        momentum_relaxed = params.get('momentum_relaxed', False)
        ...
        if strategy == 'TREND_PULLBACK':
            ...
            # Condition 1: Tendance Haussière
            if momentum_relaxed:
                 # Gold Logic: Only Price > SMA200 needed
                 is_bull_trend = (current['close'] > current['SMA_200'])
            else:
                 # Standard Logic: Price > SMA200 AND EMA50 > SMA200
