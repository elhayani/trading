...
class AILogic:
    @staticmethod
    def detect_market_regime(btc_data, news_sentiment_pct):
        """
        V4 HYBRID: Détecte le régime de marché pour adapter la stratégie
        Returns: 'EXTREME_BEAR' | 'NORMAL_BEAR' | 'BULL'
        """
        if len(btc_data) < 168:  # Besoin 7j minimum
            return 'BULL'  # Default safe
        
        # BTC performance 7 jours
...
if __name__ == "__main__":
    ...
    print(f"\n🚀 BACKTEST V4 HYBRID (ANNÉE {target_year})")
    print("💡V4 = Auto-switch V1 (bear extrême) / V3 (normal/bull)")
