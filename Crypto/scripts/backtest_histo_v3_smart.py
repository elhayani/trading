...
class AILogic:
    ...
    def ask_confirmation(symbol, date_str, indicators, patterns):
        ...
        🎯 MISSION: Tu es un Risk Manager RATIONNEL (pas ultra-conservateur).
        Ce signal a passé des filtres techniques solides.
        Analyse le CONTEXTE NEWS pour détecter les VRAIS risques catastrophiques.
        
        ⛔ CANCEL UNIQUEMENT si:
        1. **NEWS MAJORITAIREMENT TRÈS NÉGATIVES** (> 70% négatifs)
        2. News mentionnent: hack majeur, exchange bankruptcy, regulatory ban
        3. Divergence EXTRÊME: Technique OK MAIS catastrophe macro
        
        ✅ CONFIRM (défaut) si:
        - News neutres, mixtes, ou légèrement négatives
        - Technique solide (RSI oversold + volume + trend)
        - **En cas de doute → CONFIRM** (trust la technique)
        
        🚀 BOOST (Levier x2) si:
        - News TRÈS positives (> 70% positifs)
        - ET technique excellent (RSI 35-45, Vol > 2x)
        - ET pas de red flags
        
        💡 PHILOSOPHIE V3:
        - **Technique prime sur news** (sauf catastrophe évidente)
        - Le marché surréagit aux news → opportunités
        - Only cancel si vraie menace existentielle
        
        RÉPONSE JSON:
        {{ "decision": "CONFIRM" | "CANCEL" | "BOOST", "reason": "Courte explication" }}
        """
