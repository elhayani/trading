    ...
    for pair, config in CONFIGURATION.items():
        ...
        if signal:
            ...
            try:
                ...
                # Log to DynamoDB (Empire Dashboard) regardless of decision (optional, but requested for 'Live Feed')
                # Only logging CONFIRMED trades as 'OPEN' makes sense for now based on user request "Green/Red/Blue"
                if ai_decision['decision'] == 'CONFIRM':
...
def log_trade_to_dynamo(signal_data):
    try:
        ...
        # Convert floats to Decimal or String for DynamoDB
        # Using String is safest for simple JSON dumping later
        item = {
            'TradeId': trade_id,
            'Timestamp': timestamp,
            'AssetClass': 'Indices',
            'Pair': signal_data.get('pair'),
            'Type': signal_data.get('signal'), # LONG/SHORT
