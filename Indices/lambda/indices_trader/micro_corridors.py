...
def check_volume_veto(symbol: str, current_volume: float, avg_volume: float) -> dict:
    """
    🛑 VETO DE VOLUME - Point 3 de la Check-list Empire V5.1
    
    Un corridor de temps n'est valide que si le volume confirme la session.
    Évite les "fausses ouvertures" (fakeouts).
    
    Args:
        symbol: Le symbole (ex: '^NDX', 'EURUSD')
        current_volume: Volume actuel de la bougie/période
        avg_volume: Volume moyen (généralement SMA 20 du volume)
        
    Returns:
        Dict avec:
        - veto: bool - True si le trade doit être BLOQUÉ
        - volume_ratio: float - Ratio current/avg
        - min_required: float - Seuil minimum du corridor
        - reason: str - Explication pour le logging
    """
...
def get_adaptive_params(symbol: str, base_params: dict = None) -> dict:
    """
    ...
    Args:
        ...
        base_params: Paramètres de base (optionnel, pour override)
        
    Returns:
        Dict avec tous les paramètres adaptés:
        - tp_multiplier, sl_multiplier, risk_multiplier
        - rsi_threshold, min_volume_ratio, max_trades
        - scalping_mode, regime, corridor_name
