...
class AILogic:
    ...
    def ask_confirmation(symbol, date_str, indicators, patterns):
        ...
        🎯 MISSION CRITIQUE: Tu es un Risk Manager ULTRA-STRICT.
        Ce signal a passé les filtres techniques de base, MAIS tu dois maintenant analyser 
        le CONTEXTE NEWS pour détecter les PIÈGES.
        
        ⛔ TU DOIS OBLIGATOIREMENT REJETER (CANCEL) si:
        1. **NEWS MAJORITAIREMENT NÉGATIVES** (>= 50% des articles)
           → Indicateur le PLUS IMPORTANT
        2. News mentionnent: SEC, régulation, hack, sell-off, downgrades, FUD
        3. Divergence critique: Technique positif MAIS News très négatives
        4. RSI > 55 (surachat) ou RSI < 30 (survente excessive)
        5. Volume Ratio < 1.5x (conviction faible)
        
        ✅ CONFIRMER (CONFIRM) uniquement si:
        - News neutres OU légèrement positives (pas de red flags)
        - ET RSI entre 35-55
        - ET Volume > 1.5x
        
        🚀 BOOST (Levier x2) si:
        - News TRÈS positives (majorité d'articles positifs)
        - ET RSI optimal 40-50
        - ET Volume > 2.5x
        - ET Trend RISING
        
        ⚠️ RÈGLES ABSOLUES:
        - **Si NEWS négatives → CANCEL AUTOMATIQUEMENT** (même si technique bon)
        - Si doute sur sentiment news → CANCEL par précaution
